...
$message_content = $message->message_image ?
    str_replace('###attachment###','<img src="'.Config::get('constants.SITE_BASE_URL') . Config::get('constants.MESSAGE_IMG_FOLDER') . '/' . $message->message_image . '?' . rand(1, 1000).'" class="message_image_img">',$message->message_content) : $message->message_content;
...
<div class="row {{ $start_end }} {{$current_users_message_class}}">
    <div class="col-8 message_content">
        ...
        <span>{{$current_users_message ? 'Me' : $message->user_first_name }} {{date('D d h:i a',strtotime($message->message_created_time ))}}</span>
    </div>
</div>