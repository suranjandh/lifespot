...
class TaskBeneficiary extends Task
{
    public $task_ids_no_beneficiary = [
        24
    ];
    ...
    public function get_tasks_set()
    {
        ...
        if ($beneficiaries == null || count($beneficiaries) == 0) {
            $task_skip_keys = TaskSkip::task_skip_keys($this->task_ids_no_beneficiary);
            $task = Task::find($this->task_ids_no_beneficiary[0]);
            if ($task) {
                $task->task_skip_sub_category = 'no_beneficiaries_';
            ...
            foreach ($beneficiaries as $beneficiary) {
                $empty_log_row = EmptyLog::where(
                    ['empty_log_table_name' => 'members',
                        'empty_log_row_id' => $beneficiary->member_id])->first();
                if ($empty_log_row) {
                    ...
                    if ($empty_fields_count >= 3) {
                        ...
                        $task->task_skip_sub_category = 'beneficiary_full_' . $beneficiary->member_id;
                        if (self::is_task($task, $task_skip_keys)) {
                            $task_messages[] = $this->prepare_task_message($task, $beneficiary);
                        } elseif (self::is_skipped_task($task, $task_skip_keys)) {
                            $task_skip_messages[] = $this->prepare_task_message($task, $beneficiary);
                        }
                        ...
                        foreach ($empty_log_fields as $field) {
                            ...
                            $task->task_skip_sub_category = 'beneficiary_single_' . $beneficiary->member_id . '|' . $field;
                            if (self::is_task($task, $task_skip_keys)) {
                                $task_messages[] = $this->prepare_empty_field_task_message($task, $empty_log_row, $beneficiary, $field);
                            } elseif (self::is_skipped_task($task, $task_skip_keys)) {
                                $task_skip_messages[] = $this->prepare_empty_field_task_message($task, $empty_log_row, $beneficiary, $field);
                            }
                        }
                    }
                    ...
                    if ($empty_log_row->empty_log_image_empty_field != '') {
                        ...
                        $task->task_skip_sub_category = 'beneficiary_single_' . $beneficiary->member_id . '|' . 'empty_log_image_empty_field';
                        if (self::is_task($task, $task_skip_keys)) {
                            $task_messages[] = $this->prepare_empty_field_task_message($task, $empty_log_row, $beneficiary, $field, true);
                        } elseif (self::is_skipped_task($task, $task_skip_keys)) {
                            $task_skip_messages[] = $this->prepare_empty_field_task_message($task, $empty_log_row, $beneficiary, $field, true);
                        }
                    }
                }
            }
    ...
    {
        $task = $task->toArray();
        $table_data = EmptyLog::get_table_data('Beneficiary');
        $sub_category_name = 'beneficiary';
        $message_data = array(
