...
class TaskMember extends Task
{
    ...
    public function get_tasks_set()
    {
        ...
        $members = Member::only_member_type_members();//auth()->user()->members;
        ...
        foreach ($members as $member) {
            ...
            if ($empty_log_row) {
                ...
                if ($empty_fields_count >= 3) {
                    ...
                    $task->task_skip_sub_category = 'member_full_' . $member->member_id;
                    ...
                    foreach ($empty_log_fields as $field) {
                        ...
                        $task->task_skip_sub_category = 'member_single_' . $member->member_id . '|' . $field;
                        if (self::is_task($task, $task_skip_keys)) {
                ...
                if ($empty_log_row->empty_log_image_empty_field != '') {
                    ...
                    $task->task_skip_sub_category = 'member_single_' . $member->member_id . '|' . 'empty_log_image_empty_field';
                    if (self::is_task($task, $task_skip_keys)) {
