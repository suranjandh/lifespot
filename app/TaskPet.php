...
class TaskPet extends Task
{
    public $task_ids_no_pets = [
        32
    ];
    ...
    public function get_tasks_set()
    {
        ...
        if ($pets == null || count($pets) == 0) {
            $task_skip_keys = TaskSkip::task_skip_keys($this->task_ids_no_pets);
            $task = Task::find($this->task_ids_no_pets[0]);
            if ($task) {
                $task->task_skip_sub_category = 'no_pets_';
                $task_message = $this->prepare_task_message($task, (object)array('pet_id'=>0));
            ...
            foreach ($pets as $pet) {
                $empty_log_row = EmptyLog::where(
                    ['empty_log_table_name' => 'pets',
                        'empty_log_row_id' => $pet->pet_id])->first();
                if ($empty_log_row) {
                    ...
                    if ($empty_fields_count >= 3) {
                        ...
                        $task->task_skip_sub_category = 'pet_full_' . $pet->pet_id;
                        if (self::is_task($task, $task_skip_keys)) {
                            $task_messages[] = $this->prepare_task_message($task, $pet);
                        } elseif (self::is_skipped_task($task, $task_skip_keys)) {
                            $task_skip_messages[] = $this->prepare_task_message($task, $pet);
                        }
                        ...
                        foreach ($empty_log_fields as $field) {
                            ...
                            $task->task_skip_sub_category = 'pet_single_' . $pet->pet_id . '|' . $field;
                            if (self::is_task($task, $task_skip_keys)) {
                                $task_messages[] = $this->prepare_empty_field_task_message($task, $empty_log_row, $pet, $field);
                            } elseif (self::is_skipped_task($task, $task_skip_keys)) {
                                $task_skip_messages[] = $this->prepare_empty_field_task_message($task, $empty_log_row, $pet, $field);
                            }
                        }
                    }
                    ...
                    if ($empty_log_row->empty_log_image_empty_field != '') {
                        ...
                        $task->task_skip_sub_category = 'pet_single_' . $pet->pet_id . '|' . 'empty_log_image_empty_field';
                        if (self::is_task($task, $task_skip_keys)) {
                            $task_messages[] = $this->prepare_empty_field_task_message($task, $empty_log_row, $pet, $field, true);
                        } elseif (self::is_skipped_task($task, $task_skip_keys)) {
                            $task_skip_messages[] = $this->prepare_empty_field_task_message($task, $empty_log_row, $pet, $field, true);
                        }
                    }
                }
            }
    ...
    public function prepare_task_message($task, $member)
    {
        ...
        $message_data['open_data']['member-id'] = $pet_id;
        $message_data['open_data']['member-type'] = 'pet';
        $message_data['task_row']['task_button_text'] .= $member->pet_id  > 0 ? " " . $member->pet_name:"";
        return $message_data;
    }
    ...
    public function prepare_empty_field_task_message($task, $empty_log_row, $member, $field, $image_field = false)
    {
        $pet_id = $member->pet_id ;
        $task = $task->toArray();
        $table_data = EmptyLog::get_table_data('pet');
        $sub_category_name = 'pet';
        $message_data = array(
        ...
        $message_data['open_data']['member-id'] = $pet_id;
        $message_data['open_data']['member-type'] = 'pet';
        $message_data['task_row']['task_button_text'] .= " " . $member->pet_name;
