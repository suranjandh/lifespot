...
class TaskProfile extends Task
{
    // 8
    ...
    public function get_tasks_set()
    {
        ...
        $empty_log_row = EmptyLog::where(
            ['empty_log_table_name' => 'profiles',
                'empty_log_row_id' => $profile_found->profile_id])->first();
        if ($empty_log_row) {
    ...
    public function prepare_task_message($task)
    {
        ...
        $message_data['open_data']['member-type'] = 'profile';
        return $message_data;
    }
    ...
    {
        $task = $task->toArray();
        $table_data = EmptyLog::get_table_data('Profile');
