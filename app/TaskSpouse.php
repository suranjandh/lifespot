...
class TaskSpouse extends Task
{
    public $task_ids_no_spouse = [
        45
    ];
    // 10
    ...
    public function get_tasks_set()
    {
        ...
        if (Profile::is_married()) {
            if ($spouse_found == null) {
                $task_skip_keys = TaskSkip::task_skip_keys($this->task_ids_no_spouse);
                $task = Task::find($this->task_ids_no_spouse[0]);
                if ($task) {
                    $task->task_skip_sub_category = 'no_spouse_';
                ...
                $empty_log_row = EmptyLog::where(
                    ['empty_log_table_name' => 'members',
                        'empty_log_row_id' => $spouse_found->member_id])->first();
                if ($empty_log_row) {
                    ...
                    if ($empty_fields_count >= 3) {
                        ...
                        $task->task_skip_sub_category = $spouse_found->member_id;
                        ...
                        foreach ($empty_log_fields as $field) {
                            ...
                            $task->task_skip_sub_category = $spouse_found->member_id . '|' . $field;
                            /* if (!in_array($task->task_id . '||' . $task->task_skip_sub_category, $task_skip_keys)) {
                    ...
                    if ($empty_log_row->empty_log_image_empty_field != '') {
                        ...
                        $task->task_skip_sub_category = $spouse_found->member_id . '|' . 'empty_log_image_empty_field';
                        /* if (!in_array($task->task_id . '||' . $task->task_skip_sub_category, $task_skip_keys)) {
    ...
    public function prepare_task_message($task)
    {
        ...
        $message_data['open_data']['member-type'] = 'spouse';
        return $message_data;
    }
    ...
    {
        $task = $task->toArray();
        $table_data = EmptyLog::get_table_data('Spouse');
        $sub_category_name = 'spouse';
        $message_data = array(
            ...
            if ($empty_fields_count >= 3) {
                switch ($table) {
                    ...
                    case 'spouses':
                        ...
                        if ($task) {
                            $task_message = $this->prepare_task_message($task);
                            $spouse_obj = new SpouseModel();
                            $spouse = $spouse_obj->get_spouse_by_spouse_user_id($this->user_id);
                            $task_message['open_data']['class'] = 'member_content_box';
                            $task_message['open_data']['member-member-id'] = $spouse['spouse_id'];
